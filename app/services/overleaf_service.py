...
'''
    ...
    _custom_templates_by_user: dict[str, str] = {}
    _selected_template_by_user: dict[str, str] = {}
    ...
    def set_custom_template(
        ...
        validate_required_sections: bool = True,
    ) -> dict:
        """Allow user to provide their own custom LaTeX template."""
        if validate_required_sections:
            # Basic validation for user-provided custom templates only.
        ...
        cls = type(self)
        if user_id:
            cls._custom_templates_by_user[user_id] = latex_code
            cls._selected_template_by_user[user_id] = template_name or "custom"
        else:
            cls._custom_template = latex_code
            cls._current_template_name = template_name or "custom"
    ...
    def get_current_template(self, user_id: Optional[str] = None) -> str:
        """Get the current template (custom or default)."""
        cls = type(self)
        if user_id and user_id in cls._custom_templates_by_user:
            return cls._custom_templates_by_user[user_id]
        return cls._custom_template or self.DEFAULT_LATEX_TEMPLATE

    def get_current_template_name(self, user_id: Optional[str] = None) -> str:
        """Get currently selected template name for UI state."""
        cls = type(self)
        if user_id:
            return cls._selected_template_by_user.get(user_id, "default")
        return cls._current_template_name or "default"
    ...
    async def generate_latex_resume(self, user_id: str, use_custom: bool = True) -> dict:
        ...
        template = self.get_current_template(user_id) if use_custom else self.DEFAULT_LATEX_TEMPLATE
