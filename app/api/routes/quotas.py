...
@router.get("/analyze-budget")
async def get_analyze_budget(
    current_user: User = Depends(get_current_user),
):
    """Get per-run and monthly analyze token budget for the current user's plan."""
    return await analyze_budget_service.get_analyze_budget(current_user.id)
