...
class InternshalaScraper(BaseScraper):
    ...
    async def _fetch_html_page(
        ...
        base_url = f"/internships/{slug}-internship"
        # Internshala pagination changes often. Try a few known patterns,
        # and skip duplicate HTML payloads within the same page fetch.
        attempts: List[tuple[str, Optional[Dict[str, Any]], str]] = [
            (base_url, {"start": start, "wfh": 1}, "start+wfh"),
        ]
        if page > 1:
            attempts = [
                (base_url, {"page": page, "wfh": 1}, "page+wfh"),
                (base_url, {"start": start, "wfh": 1}, "start+wfh"),
                (f"{base_url}/page-{page}", None, "path-page"),
                (base_url, {"page": page}, "page"),
                (base_url, {"start": start}, "start"),
            ]

        seen_bodies: set[int] = set()

        for attempt_url, attempt_params, mode in attempts:
            self.log.debug(
                "Fetching page",
                url=attempt_url,
                start=start,
            ...
            body_sig = hash(resp.text)
            if body_sig in seen_bodies:
                self.log.info(
                    "Duplicate page payload, trying next pagination mode",
                    page=page,
                    mode=mode,
                    category=category,
                )
            ...
            parsed = self._parse_html(resp.text, category)
            if parsed:
                return parsed
