...
import { ArrowLeft, ExternalLink, Calendar, Star, Edit3, Building, Mail, AlignLeft, ShieldAlert, Users, ChevronRight, Briefcase, FileText, Clock } from "lucide-react";
      ...
      <div className="glass-card p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold font-display">{app?.job_title_snapshot ?? job?.title}</h1>
            <p className="text-muted-foreground mt-1 flex items-center gap-2">
              <Building className="w-4 h-4" /> {app?.company_snapshot ?? job?.company_name}
            </p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => starMut.mutate()} className="p-2 glass rounded-lg hover:bg-white/5 disabled:opacity-50 text-amber-400">
              <Star className={cn("w-5 h-5", app?.is_starred ? "fill-amber-400" : "")} />
            </button>
            <button onClick={handleEdit} className="flex items-center gap-2 px-4 py-2 glass rounded-lg text-sm hover:bg-white/5 transition-colors">
              <Edit3 className="w-4 h-4" /> Update
            </button>
            {job?.source_url && (
              <a href={job.source_url} target="_blank" rel="noopener noreferrer" className="p-2 glass rounded-lg hover:bg-white/5 text-muted-foreground">
          ...
          {isEditing ? (
            <form onSubmit={submitEdit} className="glass-card p-6 space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-border">
                <button type="button" onClick={() => setIsEditing(false)} className="px-4 py-2 glass rounded-md text-sm hover:bg-white/5">Cancel</button>
                <button type="submit" disabled={updateMut.isPending} className="px-4 py-2 bg-brand-purple text-white rounded-md text-sm hover:bg-brand-purple/90">Save Changes</button>
              </div>
            </form>
          ) : (
              ...
              {(app?.notes || app?.recruiter_name || app?.interview_date || app?.offer_salary) ? (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    {app.interview_date && <div><p className="text-xs text-muted-foreground mb-1">Interview At</p><p className="text-sm font-medium flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-brand-purple-light" /> {formatDateTime(app.interview_date)}</p></div>}
                ...
                <div className="text-center py-6 text-muted-foreground"><p className="text-sm">No additional details added yet.</p><button onClick={handleEdit} className="text-brand-purple-light hover:underline text-sm mt-2">Add details</button></div>
              )}
            </div>
          )}
        ...
        <div className="glass-card p-6">
          ...
          <div className="relative border-l border-border/80 ml-3 space-y-6">
            <div className="relative pl-6">
               <div className="absolute w-3 h-3 bg-card border-2 border-brand-purple rounded-full left-[-6.5px] top-1" />
